...
import notq.dccast.databinding.FragmentNotificationLiveBinding;
...
import notq.dccast.screens.navigation_menu.notification.adapter.AdapterNotificationLive;
import notq.dccast.util.LoginService;
import retrofit2.Call;
import retrofit2.Callback;
import retrofit2.Response;

public class FragmentNotificationLive extends OpenVideoFragment {

  private FragmentNotificationLiveBinding binding;
  private AdapterNotificationLive adapterNotificationLive;
  ...
  public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
    ...
    binding =
        DataBindingUtil.inflate(inflater, R.layout.fragment_notification_live, container, false);
    init();
  ...
  private void init() {
    ...
    adapterNotificationLive = new AdapterNotificationLive(getActivity(), position -> {

    });
    binding.recyclerView.setAdapter(adapterNotificationLive);
  ...
  private void prepareRecyclerViewForNewItems() {
    adapterNotificationLive.removeNotifications();
    isLoading = false;
    pageIndex = 1;
  }
  ...
  private void getNotifications() {
    ...
    adapterNotificationLive.showLoadMoreLoader();

    Call<ModelNotificationWrapper> call =
        apiInterface.getNotificationList("live", loginUser.getId(), pageIndex);
    ...
    call.enqueue(new Callback<ModelNotificationWrapper>() {
      ...
      public void onResponse(@NonNull Call<ModelNotificationWrapper> call,
        ...
        for (ModelNotification notification : mediaWrapper.notifications) {
          adapterNotificationLive.addNotification(notification);
        }
  ...
  private void hideLoaders() {
    ...
    adapterNotificationLive.hideLoadMoreLoader();
    adapterNotificationLive.notifyItemChanged(AdapterVideos.VIEW_TYPE_NO_DATA);
  }
