...
public class AdapterVideos extends RecyclerView.Adapter {
  ...
  private VHMainHeader mainHeader;
  private int type;

  private boolean isAll = true;

  public AdapterVideos(Fragment fragment, TabLayout.OnTabSelectedListener tabSelectedListener,
  ...
  public AdapterVideos(Fragment fragment, boolean isLive) {
    this.isLive = isLive;
    this.fragment = fragment;
    videos = new ArrayList<>();
  }
  ...
  public void updateVideo(ModelVideo modelVideo) {
    ...
    if (mainHeader != null) {
      fromCategory = mainHeader.updateVideo(modelVideo);
    }
  ...
  public void updateViewCount(ModelVideo modelVideo) {
    if (mainHeader != null) {
      mainHeader.updateVideo(modelVideo);
    }
  ...
  public void updateViewCount(ModelListHeader modelVideo) {
    if (mainHeader != null) {
      mainHeader.updateHeaderVideo(modelVideo);
    }
  }

  public void updateVideo(ModelListHeader header) {
    if (mainHeader != null) {
      mainHeader.updateHeaderVideo(header);
    }
  }
  ...
  public void removeVideo(ModelVideo modelVideo) {
    boolean fromCategory = false;
    if (mainHeader != null) {
      fromCategory = mainHeader.deleteVideo(modelVideo);
    }
  ...
  public ArrayList<ModelVideo> getPopularVideos() {
    ...
    if (mainHeader != null) {
      return mainHeader.getPopularVideos();
    }
    return videos;
  }

  public void setPopularVideos(ArrayList<ModelVideo> videos) {
    if (mainHeader != null) {
      mainHeader.setPopularVideos(videos);
    }
    notifyItemChanged(VIEW_TYPE_MAIN_HEADER);
  }
  ...
  public void hidePopularVideos() {
    isAll = false;
    if (mainHeader != null) {
      mainHeader.hidePopularVideos();
    }
  }

  public void refreshCategoryVideo() {
    if (mainHeader != null) {
      mainHeader.toggleLive(isLive);
    }
  }
  ...
  public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
    ...
    switch (viewType) {
      case VIEW_TYPE_MAIN_HEADER: {
        ...
        mainHeader = new VHMainHeader(fragment, headerBinding);
        return mainHeader;
      }
      ...
      case VIEW_TYPE_CATEGORY: {
        ...
        category = new VHCategory(fragment, categoryTabListener,
            categoryBinding);
        return category;
      }
    ...
    switch (getItemViewType(position)) {
      case VIEW_TYPE_MAIN_HEADER: {
        ...
        try {
          ...
          if (isAll) {
            if (isNeedShowHeaderPager || headers.isEmpty()) {
          ...
          Timber.e("illegalStateException ERROR: " + ex.getMessage());
        } catch (Exception ex) {
      ...
      case VIEW_TYPE_NO_DATA: {
        if (videos.isEmpty()) {
          ((VHNoData) holder).show(isLive ? fragment.getString(R.string.home_no_data_live)
              : fragment.getString(R.string.home_no_data_vod));
        } else {
